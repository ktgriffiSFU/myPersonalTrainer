...
#import "PLBarChartViewController.h"
#import "CorePlot-CocoaTouch.h"

@interface PLBarChartViewController ()
...
@implementation PLBarChartViewController
...
-(void)configureGraph {
    ...
    // 5 - Set up plot space
    NSNumber *xMin=[NSNumber numberWithInt:0];
    NSNumber *xMax = [NSNumber numberWithInt:30];
    NSNumber *yMin=[NSNumber numberWithInt:0];
    NSNumber *yMax =[NSNumber numberWithInt:600];  // should determine dynamically based on max price
    CPTXYPlotSpace *plotSpace = (CPTXYPlotSpace *) graph.defaultPlotSpace;
    plotSpace.xRange = [CPTPlotRange plotRangeWithLocation:xMin length:xMax];
    plotSpace.yRange = [CPTPlotRange plotRangeWithLocation:yMin length:yMax];
...
-(void)configurePlots {
    ...
    NSNumber *barXNumber=@(barX);
    NSArray *plots = [NSArray arrayWithObjects:self.aaplPlot, self.googPlot, self.msftPlot, nil];
    NSNumber *numbBars=@(30);
    CGFloat screenwidth = [UIScreen mainScreen].bounds.size.width;
    NSNumber *width=@(screenwidth);
    double widthdbl=[width doubleValue];
    double numBarsdbl=[numbBars doubleValue];
    double barWidthdbl=widthdbl/numBarsdbl;
    NSNumber *barWidth=@(barWidthdbl);
    for (CPTBarPlot *plot in plots) {
        plot.dataSource = self;
        plot.delegate = self;
        plot.barWidth = barWidth;
        plot.barOffset = barXNumber;
...
-(void)configureAxes {
    ...
    axisSet.xAxis.title = @"Workouts";
    axisSet.xAxis.titleTextStyle = axisTitleStyle;
    axisSet.xAxis.titleOffset = 10.0f;
    axisSet.xAxis.axisLineStyle = axisLineStyle;
    // 4 - Configure the y-axis
    axisSet.yAxis.labelingPolicy = CPTAxisLabelingPolicyNone;
    axisSet.yAxis.title = @"Weight(kg)";
